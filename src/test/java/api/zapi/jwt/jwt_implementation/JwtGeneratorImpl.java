package api.zapi.jwt.jwt_implementation;
...
import api.zapi.jwt.jwt_core.JwtAuthorizationGenerator;
import api.zapi.ZConfig;
...
public class JwtGeneratorImpl implements JwtGenerator {
    ...
    public String generateJWT(String requestMethod, URI uri, final int jwtExpiryWindowSeconds) {
        ...
        final JwtAuthorizationGenerator jwtAuthorisationGenerator = new JwtAuthorizationGenerator(jwtWriterFactory,
                jwtExpiryWindowSeconds) {

            public Option<String> generate(HttpMethod httpMethod, URI url, Map<String, List<String>> parameters,
                    AcHost acHost, Option<String> userId)
                ...
                Map<String, String[]> paramsAsArrays = Maps.transformValues(parameters,
                ...
                return Option.some(JWT_AUTH_HEADER_PREFIX
                        + encodeJwt(httpMethod, url, paramsAsArrays, userId.getOrNull(), acHost));
            }

            private String encodeJwt(HttpMethod httpMethod, URI targetPath, Map<String, String[]> params,
                    String userKeyValue, AcHost acHost)
                ...
                JwtJsonBuilder jsonBuilder = new JsonSmartJwtJsonBuilder().issuedAt(TimeUtil.currentTimeSeconds())
            ...
            private String issueJwt(String jsonPayload, ZConfig config)
                    throws JwtSigningException, JwtIssuerLacksSharedSecretException, JwtUnknownIssuerException {
                return getJwtWriter(config).jsonToJwt(jsonPayload);
            }

            private JwtWriter getJwtWriter(ZConfig config)
                    throws JwtUnknownIssuerException, JwtIssuerLacksSharedSecretException {
            ...
            private Map<String, String[]> constructParameterMap(URI uri) throws UnsupportedEncodingException {
                ...
                while (!cursor.atEnd()) {
                    NameValuePair nameValuePair = BasicHeaderValueParser.DEFAULT.parseNameValuePair(buffer, cursor,
                            QUERY_DELIMITERS);
                    if (!StringUtils.isEmpty(nameValuePair.getName())) {
                        ...
                        String[] newValues = null == oldValues ? new String[1]
        ...
        try {
            ...
            jwt = jwtAuthorisationGenerator.generate(HttpMethod.valueOf(requestMethod), uriWithoutProductContext,
                    new HashMap<String, List<String>>(), zConfig.host, zConfig.USER_NAME);
        } catch (Exception e) {
    ...
    private URI getUri(URI uri, String baseUrlString) throws URISyntaxException {
        ...
        return new URI(uri.getScheme(), uri.getUserInfo(), uri.getHost(), uri.getPort(), pathWithoutProductContext,
